...
typedef struct insert_Line {
	char label[10], opcode[10], operand[10];
	struct insert_Line* nextLine;
	int ad;
}Line;
...
typedef struct intermedFile {
	Line* ptrLabel, * ptrOpcode, * ptrOperand;
	int adLine;
	struct intermedFile* nextMidfile;
}Midfile;

int main(int argc, char* argv[]) {
	int  opt = 0, address = 0, end = 0;
	char askii[10] = { '\0' };
	char fromhexa[10] = { '\0' };
	char* filename = NULL;
	char* for16 = NULL;
	Line* head = NULL, * cur = NULL, * tail = NULL;
	Table* head2 = NULL, * cur2 = NULL, * tail2 = NULL;
	Midfile* head3 = NULL, * cur3 = NULL, * tail3 = NULL, * forOB = NULL, * forOB2 = NULL;
		...
		if (head->nextLine == NULL)
			...
			if (strcmp(line->label, "**") == 0) {
				...
				if (strcmp(line->opcode, "RESW") == 0) {
					line->ad = address;
					address = address + (atoi(line->operand) * 3);
				}
	...
	while (cur3 != NULL) {
		if (cur3 == head3) {
			printf("%s %s %s\n", cur3->ptrLabel->label, cur3->ptrOpcode->opcode,
				cur3->ptrOperand->operand);
			cur3 = cur3->nextMidfile;
		}
		printf("%d %s %s %s\n", cur3->adLine, cur3->ptrLabel->label, cur3->ptrOpcode->opcode,
			cur3->ptrOperand->operand);
		cur3 = cur3->nextMidfile;
	}printf("\n");
